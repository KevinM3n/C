	...
	if (answer[0] == 'A' &&
	    ...
	    answer[5] == 'E' )
	  {
	    if (numberofstudent == 0)
	      {
		cout << "You don't have any students in the studentlist!" << endl;
	      }
	    else
	      {
		//deletestudent(studentlist);
		  ...
		  cin >> id2; // read in the id
 cin.get();
 int xd = 0; // keep track of the list
 for (vector<student*>::iterator it = studentlist.begin(); it != studentlist.end(); ++it)
   {

     
     xd ++; 
     if ((*it)->id == id2 ) // go through the studentlist and find the one with the same id number
   {
     
     studentlist.erase(studentlist.begin() + xd-1); // delete that one
     break;      
   }
     else if (xd == numberofstudent) // if the for loop is ending and we havn't found the one, tell user it doesn't exist
       
       {
	 cout << "Could not find any student with the ID provided in studentlist." << endl;
       }
